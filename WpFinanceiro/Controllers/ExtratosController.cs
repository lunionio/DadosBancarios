...
namespace WpFinanceiro.Controllers
{
    ...
    public class ExtratosController : ControllerBase
    {
        ...
        [HttpGet("LiberarPagamento/{idCliente:int}/{codigoExterno:int}/{token}")]
        public async Task<IActionResult> LiberarPagamentoAsync([FromRoute]string token, [FromRoute]int idCliente, [FromRoute]int codigoExterno)
        {
            try
            {
                await _service.ValidateTokenAsync(token);
                if(_domain.LiberarPagamento(idCliente, codigoExterno))
        ...
        [HttpGet("BuscarSaldo/{idCliente:int}/{destino}/{token}")]
        public async Task<IActionResult> BuscarSaldoAsync([FromRoute]int idCliente, [FromRoute]string destino, [FromRoute]string token)
        {
            try
            {
                await _service.ValidateTokenAsync(token);
                var saldo = _domain.GetSaldo(idCliente, destino);
